...
# Collect enviroment variables for app
eval enviroment_variables="$(env | grep MYMONERO_WEB.*=)"

# Loop over variables
env | grep MYMONERO_WEB.*= | while read -r line; 
do
    printf "%s',\n" $line | sed "s/=/:'/" >> /usr/share/nginx/html/env_config.js
    
    # Notify the user
    printf "Env variable %s' was injected into App. \n" $line | sed "0,/=/{s//:'/}"
